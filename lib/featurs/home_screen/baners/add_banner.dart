    ...
    return BlocProvider(
      ...
      child: BlocConsumer<BannersCubit, BannersState>(
        listener: (context, state) {
          if (state is AddBannersSuccessful) {
            Navigator.pushNamed(context, Routes.homeRoute);
            showToast(content: state.message);
          } else if (state is AddBannersFailed) {
            showToast(content: state.message);
          }
        },
        builder: (context, state) {
          var cubit = BannersCubit().get(context);
          var addBannerKey = GlobalKey<FormState>();
          return Scaffold(
            backgroundColor: Colours.White,
            appBar: AppBar(
              title: Text(
                config.localization['addBanner'],
                  ...
                  Form(
                    ...
                    child: Column(
                      children: [
                        ...
                        ConditionBuilder(
                          condition: state is AddBannersLoading,
                          ifFalse: CustomBotton(
                            navigate_fun: () {
                              if (cubit.imageFile != null) {
                                if (addBannerKey.currentState!.validate()) {
                                  cubit.AddBanner(
                            ...
                            lable: config.localization['addBanner'],
                            fontSize: 20.sp,
                          ),
                        ),
                      ],
