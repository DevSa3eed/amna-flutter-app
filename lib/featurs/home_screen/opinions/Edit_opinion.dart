...
class EditOpinion extends StatelessWidget {
  const EditOpinion({required this.opinionModel, super.key});
  final Opinions opinionModel;
  @override
      ...
      child: BlocConsumer<OpinionsCubit, OpinionsState>(
        ...
        builder: (context, state) {
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text(
                config.localization['editOpinion'],
                    ...
                    child: Column(
                      children: [
                        ...
                        CustomTextFeild(
                            ...
                            type: TextInputType.text),
                        ConditionBuilder(
                            condition: state is EditOpinionsloading,
                            ifFalse: CustomBotton(
                                navigate_fun: () {
                                  if (opinionFormKey.currentState!.validate()) {
                                    cubit.EditOpinions(
                                      id: opinionModel.id!,
                                      userid: opinionModel.user!.usersId!,
                                      title: titleController.text,
                                      description: descController.text,
                                    );
                                  }
                                },
                                lable: config.localization['editOpinion'],
                                fontSize: 20.sp))
                      ],
                    ),
                  ),
